...
import { getMenuAsync, getRolePwerAsync, getUserMenuAsync } from "@/apis/system/menu";
...
export default async function auth(to: RouteLocationNormalized, from: RouteLocationNormalized, next: Next) {
  ...
  if (token) {
    ...
    if (!userStore.totalMenus?.length) {
      // const [totalMenus, menus] = await Promise.all([getMenuAsync(), getUserMenuAsync()])
      // userStore.updateTotalMenus(totalMenus, menus)
      const [menus, powers] = await Promise.all([getMenuAsync(), getRolePwerAsync({
        RoCode: userStore.userData?.roCode
      })])
      ...
      const allowedMenus = menus.filter(menu => {
        menu.type = 0
        return allowedCodes.includes(menu.muCode)
      })
      userStore.updateTotalMenus(allowedMenus, allowedMenus)
    }
  }
  ...
  // 是否在合法菜单中
  const isValidEntry = menus.find(m => isRoutePathMatch(m.muUrl, to.path))

...
import {ElTabs,ElTabPane} from 'element-plus'
  ...
  // 为配置项提供反射数据源
  const babyReflections = reactive([])


  const babyConfig = defineConfig<BrandResultModel>({
    reflect: true,
    size: 'default',
    // 获取列表的api
  ...
  const momConfig = defineConfig<BrandResultModel>({
...
<template>
  <div>
    <div>
      <el-tabs :model-value="tabValue" type="border-card"  @tab-change="e=>changeTab(e)">
       <el-tab-pane v-for="(tab, index) in tabList" :key="index" :label="tab.name" :name="tab.value" />
      </el-tabs>
    </div>
  </div>
  <div>
  {{tabValue}}
    <page-model v-if="tabValue === 1" :config='babyConfig' :reflections="babyReflections" />
    <page-model v-else :config='momConfig' :reflections="momReflections" />
  </div>
</template>

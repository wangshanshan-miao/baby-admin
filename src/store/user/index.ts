...
export const useUserStore = defineStore(STORE_NAME, {
  ...
  actions: {
    ...
    updateTotalMenus(totalMenus: MenuModel[], menus: MenuModel[]) {
      ...
      menus.forEach(menu => {
        if (menu.muUrl) {
          tagViewsTitleMap.set(menu.muUrl, menu.muName)
        }
      })
      ...
      this.groupTreeMenus = Object.keys(groupMenus).reduce((memo, key) => {
        memo[key] = tree(groupMenus[key], 'muCode', 'pmuCode', 'childs')
        return memo
    ...
    updateCurrentMenu() {
      ...
      const matchMenu = menus.find(menu => isRoutePathMatch(useTestPath, menu.muUrl))
      ...
      console.log('currentUseMenus', this.currentUseMenus)
    }
  }
})

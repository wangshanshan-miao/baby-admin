...
<script setup lang="ts">
  ...
  const treedMenus = computed(() => {
    return tree(totalMenus.value, 'muCode', 'pmuCode', 'childs')
  })
  ...
  const onAddChildMenu = (menu: MenuModel, index: number) => {
    menuFormRef.value.resetValue(menu.muCode)
  }
  ...
  const onDel = async (menu: MenuModel, index: number) => {
    ...
    await delMenuAsync({ muCode: menu.muCode })
...
<template>
  <div :class="BEMSpace">
    ...
    />
    <menu-form ref="menuFormRef" :menus="treedMenus" @refresh="getMenus" />
    <func-form ref="funcFormRef" @refresh="getMenus" />
  </div>  
</template>

  ...
  <el-dialog v-model="isShow" title="菜单">
    <el-form class="menu-form" label-width="100px">
      ...
      <el-form-item label="菜单名称" prop="muName">
        <el-input v-model="formData.muName" />
      </el-form-item>
      <el-form-item label="菜单图标" prop="muIcon">
        <el-row class="menu-icon">
          <el-col v-for="(item) in icons" :key="item.icon" :span="4">
            <jz-icon
              :icon="item.icon"
              :class="['svg-box', {'selected': formData.muIcon === item.icon}]"
          ...
          <el-col :span="4">
            <div
              :class="['svg-box', {'selected': formData.muIcon === ''}]"
              @click="onSetIcon('')"
            >
              <span>无</span>
            </div>
          ...
          <el-col :span="6">
            <el-input size="small" v-model="formData.muIcon" />
          </el-col>
        </el-row>
      </el-form-item>
      ...
      <el-form-item label="菜单URL" prop="muUrl">
        <el-input v-model="formData.muUrl" />
      </el-form-item>
      <el-form-item>
...
const onMenuSelectChange = (e: string[]) => {
  formData.value.pmuCode = e[e.length - 1]
}

const onSetIcon = (icon: string) => {
  formData.value.muIcon = icon
}

const resetValue = (pmuCode: number) => {
  isShow.value = true
  formData.value = {
    type: MenuType.业务菜单,
    menuLevel: 0,
  ...
  if (pmuCode) {
    pIdList.value = [pmuCode]
    formData.value.pmuCode = pmuCode
  } else {
    pIdList.value = []
  }
}
...
const setValue = (setFormData: AnyObject) => {
  ...
  if (formData.value.pmuCode) {
    pIdList.value = [formData.value.pmuCode]
  } else {
    pIdList.value = []
  }
}

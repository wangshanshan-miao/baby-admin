...
const onToggleCollapse = (collapse: boolean) => {
  appStore.toggleBusinessCollapse()
}
...
<template>
  <div :class="BEMSpace">
    ...
    <el-scrollbar tag="ul" height="100%" :class="[createBEMName('root-menus'), { empty: !isMenusReady }]">
      <li v-for="(menu, index) in currentUseMenus" :key="menu.id" :class="[createBEMName('root-item'), { active: index === currentIndex }]" @click="onToggleRootMenu(index)">
        <jz-icon :icon="menu.muIcon" :font-size="20" :class="createBEMName('root-icon')" />
        <span :class="createBEMName('root-item-text')">{{ menu.muName }}</span>
      </li>
    </el-scrollbar>

    <!-- 二级菜单 -->
    <el-scrollbar height="100%" :class="[createBEMName('child-menus-wrap')]">
      <div :class="createBEMName('current-root-name')">{{ currentRootMenu.muName }}</div>
...
.aside-menus__root-item {
  ...
  &.active {
    ...
    font-weight: 700;
    color: #fff;
    background-color: rgb(81, 124, 255);
